...
fastifyInstance.decorate("authTokenData", {
	...
	expireInSeconds: 60 * 15, // 15 minutes in seconds
});

fastifyInstance.decorate("refreshTokenData", {
	name: "srt_" + "22ebd85ae574",
	expireInSeconds: 60 * 60 * 24 * 7, // 7 days in seconds
});

fastifyInstance.decorate("jwtVerify", (token, secretOrPrivateKey, options) => {
	try {
...
fastifyInstance.decorate("jwtSign", (payload, secretOrPrivateKey, options) => {
...
fastifyInstance.decorate(
	...
	(reply, payload, authTokenData, refreshTokenData) => {
		...
		/* Set auth and refresh reply cookies */
